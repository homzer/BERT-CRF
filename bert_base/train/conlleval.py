...
def report_notprint(counts, out=None):
    ...
    c = counts
    final_report = []
    line = ['processed %d tokens with %d phrases; ' %
            (c.token_counter, c.found_correct), 'found: %d phrases; correct: %d.\n' %
            (c.found_guessed, c.correct_chunk)]
    final_report.append("".join(line))

    if c.token_counter > 0:
        line = ['accuracy: %6.2f%%; ' %
                (100. * c.correct_tags / c.token_counter), 'precision: %6.2f%%; ' % (100. * overall.prec),
                'recall: %6.2f%%; ' % (100. * overall.rec), 'FB1: %6.2f\n' % (100. * overall.fscore)]
        final_report.append("".join(line))

    for i, m in sorted(by_type.items()):
        line = ['%17s: ' % i, 'precision: %6.2f%%; ' % (100. * m.prec), 'recall: %6.2f%%; ' % (100. * m.rec),
                'FB1: %6.2f  %d\n' % (100. * m.fscore, c.t_found_guessed[i])]
        final_report.append("".join(line))
    return final_report
