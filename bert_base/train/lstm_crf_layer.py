...
class BLSTM_CRF(object):
    ...
    def add_blstm_crf_layer(self, crf_only):
        ...
        if crf_only:
            logits = self.project_crf_layer(self.embedded_chars)  # 不加blstm，只使用crf，输入bert模型中得到的序列
        else:
            # blstm
    ...
    def crf_layer(self, logits):
        ...
        with tf.variable_scope("crf_loss"):
            trans = tf.get_variable(
                "transitions",
                shape=[self.num_labels, self.num_labels],  # 构建11*11的转移矩阵
                initializer=self.initializers.xavier_initializer())
            if self.labels is None:
                return None, trans
            else:
                log_likelihood, trans = tf.contrib.crf.crf_log_likelihood(  # 此处的log_likelihood的shape为啥是(4,)，因为4是batch_size
                    inputs=logits,
