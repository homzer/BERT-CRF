...
class BlstmCrf(object):
    ...
    def add_blstm_crf_layer(self, crf_only, name=None):
        ...
        if crf_only:
            logits = self.project_crf_layer(name=name)  # 不加blstm，只使用crf，输入bert模型中得到的序列
        else:
            # blstm
    ...
    def project_blstm_layer(self, lstm_outputs, name=None):
        """
    ...
    def project_crf_layer(self, name=None):
        ...
        :return: [batch_size, num_steps, num_tags]
        """
        with tf.variable_scope("project" if not name else "project_" + name):
            with tf.variable_scope("logits"):
                ...
                output = tf.reshape(self.embedded_chars,
                                    shape=[-1, self.embedding_dims])  # [batch_size * 202, embedding_dims]
                pred = tf.tanh(tf.nn.xw_plus_b(output, w, b))
            return tf.reshape(pred, [-1, self.seq_length, self.num_labels])

    def crf_layer(self, logits, name=None):
        """
        calculate crf loss
        :param name:
        :param logits: [batch_size, max_seq_length, num_tags]
        :return: scalar loss
        """
        with tf.variable_scope("crf_loss" if not name else "crf_loss_" + name):
            trans = tf.get_variable(
                "transitions",
                shape=[self.num_labels, self.num_labels],  # 构建转移矩阵
                initializer=self.initializers.xavier_initializer())
            if self.labels is None:
                return None, trans
            else:
                log_likelihood, trans = \
