...
from bert_cause.base.server.bert_worker import BertWorker
...
class TcpServer(threading.Thread):
    ...
    def load_model(self):
        ...
        with Pool(processes=1) as pool:
            ...
            num_labels, id2label = init_label(self.args.output_dir)
            self.num_labels = num_labels + 1  # 0 为保留位
    ...
    def _run(self, _, frontend, sink, *backend_socks):
        ...
        for idx, device_id in enumerate(device_map):
            process = BertWorker(idx, self.args, addr_backend_list, addr_sink, device_id,
                                 self.graph_path, self.id2label)
            self.processes.append(process)
            process.start()

        # start the http-service process
        # if self.args.http_port:
        #     from bert_cause.base.server.http_server import HttpProxy
            ...
            try:
                ...
                if msg == ServerCommand.terminate:
                    ...
                    sink.send_multipart([client,
                                         msg,
                                         jsonapi.dumps({**status_runtime, **self.status_args, **self.status_static}),
                                         req_id])
                else:
